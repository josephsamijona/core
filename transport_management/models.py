...
class Trip(models.Model):
    ...
    destination = models.CharField(max_length=255, help_text="Lieu d'arrivée", default='A')
    trip_date = models.DateTimeField(default=timezone.now, help_text="Date et heure du voyage")
    status = models.CharField(max_length=50, default='scheduled', help_text="Statut du voyage (ex: scheduled, in_progress, completed, cancelled)")
...
class TripStatus(models.Model):
    trip_status_id = models.AutoField(primary_key=True,default='')
    trip = models.ForeignKey(Trip, on_delete=models.CASCADE, help_text="Lien vers le trajet concerné")
    vehicle = models.ForeignKey(Vehicle, on_delete=models.CASCADE, help_text="Bus affecté au trajet")
    driver = models.ForeignKey(Driver, on_delete=models.CASCADE, help_text="Chauffeur affecté au trajet", default='')
    status = models.CharField(max_length=50, choices=[('ongoing', 'En cours'), ('completed', 'Terminé'), ('canceled', 'Annulé')], default='ongoing')
    ...
    updated_at = models.DateTimeField(auto_now=True, help_text="Dernière mise à jour de l'état du trajet")
